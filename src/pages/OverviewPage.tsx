    ...
    : status === 'inactive'
      ...
      : status === 'completed'
        ...
        : status === 'trash'
          ...
          : 'Room to focus.';
  const desc = focus
    ? 'Your chosen priorities. Start with one small step.'
    : status === 'inactive'
      ? 'Past deadlines and things you’ve set aside.'
      : status === 'completed'
        ? 'A record of the things you finished.'
        : status === 'trash'
          ? 'Restore what you need. Permanently delete only when you’re ready.'
  ...
  return (
    <div className="page">
            ...
            <span className="stat-value">
              {String(
                active.filter(
                  (d) =>
                    d.dueAt &&
                    Date.parse(d.dueAt) > now &&
                    Date.parse(d.dueAt) - now < 7 * 86400000,
                ).length,
              ).padStart(2, '0')}
            </span>
      ...
      <div className="filter-bar">
          ...
          onClick={async () => {
            ...
            if (
              ...
              (await setPreferences({
                ...prefs,
                savedViews: [...prefs.savedViews, { name: name.trim(), boardId, search, priority }],
              }))
            )
              toast('View saved.');
          }}
        ...
        <button
          className={'icon-button ' + (selecting ? 'selected' : '')}
          aria-label="Select multiple deadlines"
          onClick={() => {
      ...
      {prefs.savedViews.length > 0 && (
        <div className="saved-views">
          {prefs.savedViews.map((v, i) => (
            <button
              key={i}
              onClick={() => setParams({ board: v.boardId, q: v.search, priority: v.priority })}
            >
              <Bookmark size={12} />
              {v.name}
            </button>
      ...
      {selecting && (
        <div className="bulk-bar">
          ...
          <button className="text-button" onClick={() => setSelection(items.map((d) => d.id))}>
            Select all visible
          </button>
          {status !== 'trash' && (
            <>
      ...
      {items.length ? (
        view === 'grid' ? (
          <div className="deadline-grid">
            {items.map((d) => (
              <DeadlineCard
                ...
                onSelect={() =>
                  setSelection((s) =>
                    s.includes(d.id) ? s.filter((id) => id !== d.id) : [...s, d.id],
                  )
                }
              />
            ))}
          ...
          <div className="deadline-list">
            {items.map((d) => (
              <div key={d.id} className="deadline-list-row">
                {selecting && (
                  <input
                    type="checkbox"
        ...
        <EmptyState
          title={
            focus
              ? 'Choose a little less to focus on.'
              : search || boardId || priority
                ...
                : status === 'active'
                  ...
                  : status === 'trash'
                    ? 'Trash is empty.'
                    : status === 'completed'
                      ? 'Your finished work will live here.'
                      : 'Nothing set aside.'
          }
          description={
            focus
              ? 'Open a deadline and choose “Keep in focus” to bring it here.'
              : search || boardId || priority
                ? 'Try another search or clear your filters.'
                : status === 'active'
                  ? 'Add a deadline, start from a template, or discover a conference.'
      ...
      {focus && filtered.length > items.length && (
        <p className="muted">
          Showing your first {prefs.focusLimit} priorities. Change the focus limit in Settings.
        </p>
      )}
      {status === 'active' && !focus && (
        <div className="quiet-banner">
          <div>
            ...
            <h3>A board that belongs on your desk.</h3>
            <p>Open display mode and keep what matters in view.</p>
          </div>
          <button className="button secondary" onClick={() => navigate('/display')}>
            <Monitor size={18} />
            Open display
          </button>
        </div>
      ...
      {editor && (
        <DeadlineEditor key={editor.id} item={editor} onClose={() => setEditor(null)} />
      )}{' '}
      {detail && !editor && (
        <DeadlineDetail
          item={detail}
      ...
      {emptyTrash && (
        <ConfirmDialog
          ...
          message="Every deadline in Trash will be permanently removed, including items hidden by your current filters. This cannot be undone."
          label="Empty Trash"
          onClose={() => setEmptyTrash(false)}
          onConfirm={async () => {
            for (const d of data.deadlines.filter((d) => d.status === 'trash'))
              await remove('deadlines', d);
          }}
        />
      )}
    </div>
